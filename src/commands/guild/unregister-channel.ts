...
export class UnregisterChannelCommand extends Command {
  ...
  public override registerApplicationCommands(registry: Command.Registry) {
    registry.registerChatInputCommand((builder) =>
      builder
        ...
        .setDescription("Remove a pre-approved Discord channel from bot use")
  ...
  public override async chatInputRun(
    ...
    try {
      ...
      container.logger.error("Error in unregister-channel command:", error)
      return await interaction.editReply({
        content:
          "An error occurred while processing your request. Please try again later.",
      })
  ...
  private async validateAllyCodeOwnership(
    ...
    if (!ownsAllyCode) {
      return {
        ...
        response: {
          content:
            "You can only unregister channels for guilds using your own registered ally codes.",
        },
      }
    }
  ...
  private async unregisterChannel(
    ...
    try {
      ...
      // Remove the channel (backend will clear guildChannelId from automations referencing it)
